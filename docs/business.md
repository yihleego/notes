...
超大型系统：Cassandra+ClickHouse热冷分离

如何保证消息发送不丢失，以及大量数据写库不会导致数据库崩溃？
使用 Kafka 将消息异步存到数据库，因为 Kafka 是 Topic 磁盘顺序写，使用的是 sendfile 零拷贝。（如果 Topic 数量很多，会导致大量随机写）
也可以使用 RocketMQ，主打一个有什么用什么，RocketMQ 是全局 Topic 顺序写，为了支持消息处理的灵活性和功能完整性（可靠消息投递和事务消息支持），舍弃了极致的性能追求，选择了mmap 零拷贝。
