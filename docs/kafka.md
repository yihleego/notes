...
- Kafka 适合场景：
    - 日志采集与监控： 收集海量的系统日志、用户行为轨迹、打点数据。这是 Kafka 的传统强项（如 ELK 架构中的中转）。
    - 实时流处理： 配合 Flink、Spark Streaming 或 Kafka Streams 进行实时指标计算、反欺诈检测等。
    - 数据集成： 作为底层数据总线，将数据从一个系统同步到另一个系统（如从 MySQL 到 Elasticsearch/HBase）。
    - 高吞吐场景： 当你对性能的要求远高于对单个消息“精细化管理”的要求时，选 Kafka。
- RocketMQ 适合场景：
    - 分布式事务一致性： 比如“下单成功”后要“扣减积分”，如果这两个操作在不同服务，RocketMQ 的事务消息能确保最终一致性。
    - 延时与定时任务： 电商中常见的“订单 30 分钟未支付自动取消”，使用 RocketMQ 的延时消息可以非常优雅地实现。
    - 消息过滤与重试： 如果某个消费者因为偶发错误挂了，RocketMQ 会自动根据梯度进行消息重试，确保业务不丢单。
    - 削峰填谷： 处理类似抢购、秒杀等瞬时高并发流量，防止下游数据库被压垮。