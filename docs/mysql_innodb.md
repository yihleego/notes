...
在事务可以获取表中行的排他(X)锁之前，它必须首先获取表上的(IX)锁。
...
实现方式：使用事务日志（如redo log和undo log）来保证原子性。每次事务开始时，MySQL会记录日志，确保如果发生系统崩溃，能够通过回滚未提交的事务（undo）或者重做已提交的事务（redo）来恢复数据。

### 一致性 Consistency

事务执行前后，数据库必须从一个一致的状态转变到另一个一致的状态。所有约束（主键、外键、唯一性、检查约束等）在事务完成后都必须被满足。

实现方式：依靠 数据库约束（constraints）、触发器（triggers）、外键规则 和 事务机制 共同保证。如果事务中任何一步导致一致性被破坏，系统会回滚事务以保持一致性。

### 隔离性 Isolation

多个事务并发执行时，一个事务的中间状态对其他事务是不可见的。不同事务之间不会相互干扰。

实现方式：

- 锁机制硬隔离：MySQL 通过加锁（行锁、表锁）来保证事务的隔离性。不同的锁策略避免了事务之间的冲突，例如，行级锁和表级锁通过控制并发访问，避免脏读、不可重复读和幻读等现象。
- 多版本控制软隔离：事务通过“版本链”和“快照”来读取正确的数据版本，而无需加锁。
...
- 日志和数据文件：MySQL 使用 InnoDB 存储引擎的 redo log 和 undo log 来保证事务的持久性。一旦事务提交，相关的数据会被写入到硬盘，并通过日志记录确保在系统崩溃后能恢复到一致状态。
- 双写缓冲区：在写入数据到磁盘之前，MySQL 使用双写缓冲区机制，确保数据不会丢失。这意味着即使在写入过程中发生崩溃，也可以确保数据的持久性。
