...
- 如果一个系统接入 Seata 服务，那么其他所有系统都要接入 Seata 服务，具有侵入性，微服务通常无关语言，若以后需要使用 Go、Python、Ruby 等语言开发微服务无法接入 Seata 如何处理？
- Seata 事务依赖本地事务，也就是说本地事务已经提交了，可能会被回滚，此时如果有修数据的需求，无法保证当前的数据是否会被全局事务回滚。
- Seata 对开发者要求极高，只有符合规范的 SQL 能被解析反向生成回滚的 Undo Log。
- 在数据库本地事务隔离级别读已提交（Read Committed）或以上的基础上，Seata（AT 模式）的默认全局隔离级别是读未提交（Read Uncommitted），在某些业务场景是无法接受的。
